...
(+ (* (- 2048 (+ 2048 0.5)) 2) 2)
; expect 1
(define high_five (/ (* (/ (* 5 5) 5) 5) 5))
; expect high_five
high_five
; expect 5
(define summersession (- (* 8 (* 8 (* 8 4))) 30))
; expect summersession
summersession
; expect 2018
(eval (define summerBear summersession))
; expect 2018
`(,'summer ,'BEAR ,summersession)
; expect (summer bear 2018)
'(hello from cs61a)
; expect (hello from cs61a)
`(come join us this summersession)
; expect (come join us this summersession)
`(or join us next summersession ,(+ summersession 1))
; expect (or join us next summersession 2019)
(begin (define thissummer summersession) thissummer)
; expect 2018
(begin (define cs61a '(great course)) (cons (car cs61a) (car (cdr cs61a))))
; expect (great . course)
(lambda (x y) (* y (/ x (* x y))))
; expect (lambda (x y) (* y (/ x (* x y))))
(define f (lambda (x y) (* y (/ x (* x y)))))
; expect f
(f thissummer summersession)
; expect 1
(f high_five thissummer)
; expect 1
(define (func x y) (* x y (f x y)))
; expect func
(func (f high_five thissummer) summersession)
; expect 2018
(or (even? (- summersession 1)) #t (/ 1 0))
; expect #t
(and (even? thissummer) (odd? 2) (/ 1 0))
; expect #f
(or (even? 5) (odd? 4) (/ high_five 0))
; expect Error
(and (even? summersession) (odd? (+ summersession thissummer 1)) (/ thissummer 0))
; expect Error
(cond ((odd? (+ thissummer summersession)) 'tryhard)
      (else 'comejoinus (+ 1 thissummer)))
; expect 2019
(cond ((odd? (+ 1 high_five)) 'high_five!)
      ((= summersession thissummer) 'louder_high_five!!)
      (else 'awntoobad))
; expect louder_high_five!!
(cond ((= 2 3) 'what?)
      ((= 3 4) 'again!?)
      ((= 3 3)))
;expect #t
(define x 10)
;expect x
(let ((x 20) (y (+ x 50))) (cons (if (= (* 3 x) y) 'HappySummer
                                      'GoodluckNextYear) (cons thissummer nil)))
; expect (happysummer 2018)
(define func3 (mu (z) (* z (+ x y))))
; expect func3
(define cal (lambda (k) (define x 567) (define y 442) (func3 2)))
;expect cal
(cal 2018)
;expect 2018
